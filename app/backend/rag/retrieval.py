...
from app.backend.metrics import RETRIEVAL_LATENCY
...
def retrieve_context(query: str, k: int = 5) -> RAGContext:
    ...
    try:
        ...
        RETRIEVAL_LATENCY.labels(status="success").observe(time.time() - start_time)
        return RAGContext(chunks=chunks)
    except Exception as e:
        RETRIEVAL_LATENCY.labels(status="error").observe(time.time() - start_time)
